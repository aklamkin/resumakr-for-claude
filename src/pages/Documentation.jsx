...
const isAuth = await base44.auth.isAuthenticated(); // returns Promise<boolean>
...
// Logout
base44.auth.logout(); // Clears session and reloads page

// Redirect to login
base44.auth.redirectToLogin("/dashboard"); // Redirects to login, then returns to /dashboard
\`\`\`
...
4. For each default provider (limited to 1 to avoid rate limits):
   ...
   b. Call base44.integrations.Core.InvokeLLM({ prompt })
...
const handleActivateSubscription = async () => {
  ...
  if (appliedCoupon) {
    await base44.functions.invoke('applyCoupon', {
...
const { data: items, isLoading } = useQuery({
  ...
  queryFn: () => base44.entities.EntityName.list('order')
});
\`\`\`
...
const { data, isLoading } = useQuery({
  queryKey: ['resumes'],
  queryFn: () => base44.entities.Resume.list('-updated_date'),
  initialData: []
});
...
const { data } = useQuery({
  ...
  queryFn: () => base44.entities.Resume.filter({ status: 'active' }),
