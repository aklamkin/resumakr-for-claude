...
export default function Pricing() {
  ...
  const { data: plans = FALLBACK_PLANS, isLoading: loadingPlans } = useQuery({
    ...
    queryFn: async () => {
      const fetchedPlans = await base44.entities.SubscriptionPlan.filter({ is_active: true }, "order");
  ...
  const { data: activeCampaign, isLoading: loadingCampaign } = useQuery({
    ...
    queryFn: async () => {
      const campaigns = await base44.entities.MarketingCampaign.filter({ is_active: true });
