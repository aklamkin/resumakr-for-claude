...
export default function SettingsPrompts() {
  ...
  const { data: prompts = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return base44.entities.CustomPrompt.filter({ created_by: currentUser.email });
    },
    enabled: !loading,
  });
  ...
  const { data: providers = [] } = useQuery({
    ...
    queryFn: async () => {
      const currentUser = await base44.auth.me();
      return base44.entities.AIProvider.filter({ created_by: currentUser.email }, "order");
    },
    enabled: !loading,
  });

  const createPromptMutation = useMutation({
    mutationFn: (data) => base44.entities.CustomPrompt.create(data),
    onSuccess: () => {
  ...
  const updatePromptMutation = useMutation({
    mutationFn: ({ id, data }) => base44.entities.CustomPrompt.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["custom-prompts"] });
      resetForm();
  ...
  const deletePromptMutation = useMutation({
    mutationFn: (id) => base44.entities.CustomPrompt.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["custom-prompts"] });
  ...
  const updateProviderMutation = useMutation({
    mutationFn: ({ id, data }) => base44.entities.AIProvider.update(id, data),
    onSuccess: () => {
